...
To update the database use this command from the Web project folder (replace `AuthService` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../AuthService.Infrastructure/AuthService.Infrastructure.csproj -s AuthService.Web.csproj
```
