...
namespace MicroServicio.Conductores.Services
{
    public class DriverService : IServiceDriver
    {
        ...
        public async Task<string> AcceptRideAsync(string driverId)
        {
            ...
            {
                throw new ErrorMongo(ex.Message, "Error al actualizar el estado del conductor");
            }
            catch (Exception ex)
            {
                // Otros errores inesperados
                Console.WriteLine($"Error inesperado en AcceptRideAsync: {ex}");
                throw new ErrorMongo(ex.Message, "Error interno al procesar la solicitud");
            }
        }
        ...
        public async Task<object> FindAvailableStateAsync(Coordinates pickupLocation)
        {
            ...
            {
                throw new ErrorMongo(ex.Message, "Error al buscar conductores disponibles");
            }
            catch (Exception ex)
            {
                // Otros errores inesperados
        ...
        public async Task<DriverFound> FoundConductorAsync()
        {
            ...
            {
                //! Error de MongoDB
                throw new ErrorMongo(ex.Message, "Error al buscar o actualizar conductor disponible");
            }
            catch (Exception ex)
            {
                // Otros errores inesperados
        ...
        public async Task<object> RejectRideAsync(string driverId)
        {
            ...
            {
                //! Error de MongoDB
                throw new ErrorMongo(ex.Message, "Error al actualizar el estado del conductor (rechazo de viaje)");
            }
            catch (Exception ex)
            {
                // Otros errores inesperados
